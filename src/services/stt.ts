...
export function sttConfigured(): boolean {
  ...
  const gemini = Boolean(env.GEMINI_STT_API_KEY ?? env.GEMINI_TTS_API_KEY ?? env.AI_API_KEY);
...
async function callSttService(
  bytes: ArrayBuffer | Uint8Array<ArrayBufferLike>, filename: string, contentType: string
): Promise<TranscriptionResult> {
  if (!env.STT_URL || !env.STT_API_KEY) throw new Error('STT service is not configured.');
  ...
  const response = await fetch(`${env.STT_URL.replace(/\/$/, '')}/transcribe`, {
    method: 'POST', headers: { 'x-api-key': env.STT_API_KEY }, body: form, signal: AbortSignal.timeout(120_000)
  });
  if (!response.ok) throw new Error(`STT service ${response.status}: ${(await response.text()).slice(0, 250)}`);
...
async function callGeminiStt(
  bytes: ArrayBuffer | Uint8Array<ArrayBufferLike>, contentType: string
): Promise<TranscriptionResult> {
  ...
  const response = await fetch(
    ...
    {
      method: 'POST',
      headers: { 'x-goog-api-key': route.apiKey, 'content-type': 'application/json' },
      body: JSON.stringify({
        contents: [{ role: 'user', parts: [
          ...
          { inlineData: { mimeType: contentType || 'audio/wav', data: base64 } }
        ] }],
        generationConfig: {
  ...
  if (!response.ok) throw new Error(`Gemini STT ${response.status}: ${raw.slice(0, 300)}`);
  ...
  const result = JSON.parse(cleanJsonCandidate(output)) as { text?: string; language?: string };
  if (!result.text?.trim()) throw new Error('Gemini STT detected no speech.');
  return { text: result.text.trim(), language: result.language?.trim() || undefined };
}
...
async function callStt(
  ...
  if (size > env.MAX_AUDIO_BYTES) throw new Error(`Audio is too large. Maximum is ${Math.floor(env.MAX_AUDIO_BYTES / 1024 / 1024)} MB.`);
  ...
  if (env.STT_URL && env.STT_API_KEY) {
    try { return await callSttService(bytes, filename, contentType); }
    catch (error) { console.warn('STT service failed; trying Gemini fallback:', error instanceof Error ? error.message : error); }
  }
  return callGeminiStt(bytes, contentType);
}

export async function transcribeAudioBytes(bytes: Uint8Array<ArrayBufferLike>, filename='voice.wav', contentType='audio/wav') {
  return callStt(bytes, filename, contentType);
}

export async function transcribeDiscordAttachment(attachment: DiscordAttachment): Promise<TranscriptionResult> {
  if (!looksLikeAudio(attachment)) throw new Error('The selected attachment is not an audio file.');
  if ((attachment.size ?? 0) > env.MAX_AUDIO_BYTES) throw new Error(`Audio is too large. Maximum is ${Math.floor(env.MAX_AUDIO_BYTES / 1024 / 1024)} MB.`);
  const audioResponse = await fetch(attachment.url, { signal: AbortSignal.timeout(20_000) });
  if (!audioResponse.ok) throw new Error(`Could not download the Discord audio attachment (${audioResponse.status}).`);
  return callStt(await audioResponse.arrayBuffer(), attachment.filename, attachment.content_type ?? 'application/octet-stream');
}
