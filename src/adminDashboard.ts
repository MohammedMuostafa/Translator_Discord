...
<div class="shell">
  ...
  <main class="main">
    ...
    <section id="voice" class="section"><div class="card"><h3>Live Voice Tuning</h3><p class="sub">Tune intelligence vs speed and control when TD AI decides you finished speaking.</p><div class="voice-grid">
      ...
      <div class="field"><label>End-of-speech delay</label><div class="range-line"><input id="silenceRange" type="range" min="200" max="1500" step="50" /><input id="silence" type="number" min="200" max="5000" /></div></div>
      <div class="wide notice">Recommended: <b>low</b> thinking + <b>550–750ms</b> silence for natural conversation. Lower values answer faster but can cut you off during pauses.</div>
    ...
    <section id="display" class="section"><div class="grid">
      <div class="card half"><h3>Discord readability</h3><p class="sub">Discord controls its own font, so the bot cannot force a custom font size. TD AI v3.9 instead uses stronger headings, spacing, RTL isolation, and clearer answer sections.</p><div class="rtl-demo"><strong>السؤال بالعربي</strong><br>إيه الفرق بين <bdi>Smart Contract</bdi> و <bdi>Wallet</bdi>؟<br><br><strong>الرد المقترح — English</strong><br><bdi>A smart contract is code, while a wallet manages keys and signs transactions.</bdi></div></div>
      <div class="card half"><h3>Smart Answer controls</h3><p class="sub">The Discord answer panel now supports regeneration, shortening, more detail, and direct editing with a modal before you copy the final reply.</p><div class="code">🔄 Change Answer   ✂️ Shorter\n🧠 More Detail     ✏️ Edit Answer\n✅ Use This Reply  🔊 Listen</div></div>
    </div></section>
  </main>
</div>
...
function render(){const c=state.config;$('#providerCount').textContent=c.providers.length;$('#routeCount').textContent=Object.keys(c.routes||{}).length;
  $('#routes').innerHTML=c.tasks.map(task=>{const route=c.routes[task.id]||{};const ps=providersFor(task.kind);const options=ps.map(p=>'<option value="'+p.id+'" '+(route.providerId===p.id?'selected':'')+'>'+p.name+'</option>').join('');return '<div class="route"><div><div class="label">'+task.label+'</div><div class="tiny">'+task.id+'</div></div><div class="field"><label>Provider</label><select data-provider="'+task.id+'">'+options+'</select></div><div class="field"><label>Model</label><input data-model="'+task.id+'" value="'+(route.model||defaultModel(task.id))+'" /></div><button class="btn secondary" onclick="saveRoute(\''+task.id+'\')">Save</button></div>'}).join('');
  $('#providerList').innerHTML=c.providers.map(p=>'<div class="provider"><div><strong>'+p.name+'</strong><span>'+p.id+'</span></div><div>'+p.kind+'</div><div><span>'+(p.apiUrl||'Gemini native API')+'</span></div><div>'+p.apiKeyHint+'</div><div>'+(p.builtIn?'Built-in':'<button class="btn danger" onclick="removeProvider(\''+p.id+'\')">Delete</button>')+'</div></div>').join('');
  $('#thinking').value=c.voice.thinkingLevel;$('#liveVoice').value=c.voice.liveVoice;$('#ttsVoice').value=c.voice.ttsVoice;$('#silence').value=c.voice.silenceMs;$('#silenceRange').value=Math.min(1500,c.voice.silenceMs);
}
async function load(){state.config=await api('/admin/api/config');render()}
async function saveRoute(task){try{await api('/admin/api/routes/'+task,{method:'PUT',body:JSON.stringify({providerId:document.querySelector('[data-provider="'+task+'"]').value,model:document.querySelector('[data-model="'+task+'"]').value})});toast('Route saved');await load()}catch(e){toast(e.message,true)}}
async function removeProvider(id){if(!confirm('Delete this provider? Routes using it will return to environment defaults.'))return;try{await api('/admin/api/providers/'+id,{method:'DELETE'});toast('Provider deleted');await load()}catch(e){toast(e.message,true)}}
window.saveRoute=saveRoute;window.removeProvider=removeProvider;
...
$('#saveVoice').onclick=async()=>{try{await api('/admin/api/voice',{method:'PUT',body:JSON.stringify({thinkingLevel:$('#thinking').value,silenceMs:Number($('#silence').value),liveVoice:$('#liveVoice').value,ttsVoice:$('#ttsVoice').value})});toast('Voice settings saved — rejoin voice to apply model/session changes');await load()}catch(e){toast(e.message,true)}};
