...
<section id="routing" class="section"><div class="card"><h3>AI Routing</h3><p class="sub">TD manages the built-in Gemini fallback chains automatically. Pick the provider; built-in model chains are read-only and fail over on quota/rate-limit errors.</p><div id="routes"></div></div></section>
...
<div class="grid">
  ...
  <div class="card half"><h3>Conversation behavior</h3><p class="sub">Wake word has been removed. TD listens to allowed human speakers automatically and responds after each completed utterance.</p><div class="notice"><strong>Always listening</strong><br/>No need to say TD first. Use <code>Who can talk</code> to choose Everyone or Owner only.</div></div>
</div></section>
</main>
</div>
<div class="toast" id="toast"></div>
...
$('#routes').innerHTML=runtime.tasks.filter(t=>t.id!=='voice_translate').map(t=>{const r=runtime.routes[t.id]||{};return '<div class="route"><div><strong>'+esc(t.label)+'</strong><div class="sub" style="margin:3px 0 0">'+esc(t.id)+'</div></div><div class="field"><label>Provider</label><select data-provider="'+esc(t.id)+'">'+providersFor(t.kind).map(p=>'<option value="'+esc(p.id)+'" '+(r.providerId===p.id?'selected':'')+'>'+esc(p.name)+'</option>').join('')+'</select></div><div class="field"><label>Model chain '+(String(r.providerId||'').startsWith('env-')?'• Auto-managed':'')+'</label><input data-model="'+esc(t.id)+'" value="'+esc(r.model||'')+'" '+(String(r.providerId||'').startsWith('env-')?'readonly':'')+'/></div><button class="btn secondary" data-save-route="'+esc(t.id)+'">Save</button></div>'}).join('');
