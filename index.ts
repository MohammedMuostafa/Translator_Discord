...
function ephemeralError(message: string) {
  return {
    ...
    data: { content: `❌ ${message}`, flags: MessageFlags.Ephemeral, allowed_mentions: { parse: [] } }
  };
}
...
app.post('/interactions', verifyKeyMiddleware(env.DISCORD_PUBLIC_KEY), async (req, res) => {
  ...
  if (interaction.type === InteractionType.MessageComponent) {
    ...
    if (customId.startsWith('smart_reply:edit:')) {
      try {
        return res.json({ type: InteractionResponseType.Modal, data: handleSmartReplyEditModal(interaction) });
      } catch (error) {
        return res.json(ephemeralError(error instanceof Error ? error.message : 'Could not open answer editor.'));
      }
    }

    if (customId.startsWith('translate_target:')) {
      res.json({ type: InteractionResponseType.DeferredUpdateMessage });
      handleTranslateMessageSelection(interaction); return;
    }
    if (customId.startsWith('ai_action:')) {
      res.json({ type: InteractionResponseType.DeferredUpdateMessage });
      handleAiActionButton(interaction); return;
    }
    if (customId.startsWith('smart_reply:')) {
      res.json({ type: InteractionResponseType.DeferredUpdateMessage });
      handleSmartReplyButton(interaction); return;
    }
    if (customId.startsWith('ai_translate_target:')) {
      res.json({ type: InteractionResponseType.DeferredUpdateMessage });
      handleAiTranslateTarget(interaction); return;
    }
    if (customId.startsWith('listen_tts:')) {
      res.json({ type: InteractionResponseType.DeferredChannelMessageWithSource, data: { flags: MessageFlags.Ephemeral } });
      handleListenTts(interaction); return;
    }
  ...
  if (commandType === ApplicationCommandType.Message) {
    try {
      if (name === 'Translate') {
        ...
        return res.json({ type: InteractionResponseType.ChannelMessageWithSource, data: { ...payload, flags: MessageFlags.Ephemeral } });
      }
      if (name === 'TD AI') {
      ...
      return res.json(ephemeralError(error instanceof Error ? error.message : 'Unexpected error.'));
    }
    return res.json(ephemeralError('Unknown message action.'));
  }

  if (commandType !== ApplicationCommandType.ChatInput) return res.status(400).json({ error: 'Unsupported command type.' });

  if (name === 'settings') {
    try {
      const payload = await handleSettings(interaction);
      return res.json({ type: InteractionResponseType.ChannelMessageWithSource, data: { ...payload, flags: MessageFlags.Ephemeral } });
    } catch (error) { return res.json(ephemeralError(error instanceof Error ? error.message : 'Unexpected error.')); }
  }
  if (name === 'status') return res.json({ type: InteractionResponseType.ChannelMessageWithSource, data: { ...handleStatus(), flags: MessageFlags.Ephemeral } });
  if (name === 'help') return res.json({ type: InteractionResponseType.ChannelMessageWithSource, data: { ...handleHelp(), flags: MessageFlags.Ephemeral } });

  if (name === 'chat') {
    res.json({ type: InteractionResponseType.DeferredChannelMessageWithSource, data: { flags: MessageFlags.Ephemeral } }); handleChatCommand(interaction); return;
  }
  if (name === 'voicechat') {
    res.json({ type: InteractionResponseType.DeferredChannelMessageWithSource, data: { flags: MessageFlags.Ephemeral } }); handleVoiceChatCommand(interaction); return;
  }
  if (name === 'ai') {
    res.json({ type: InteractionResponseType.DeferredChannelMessageWithSource, data: { flags: MessageFlags.Ephemeral } }); handleAiSlash(interaction); return;
  }
  if (name === 'translate') {
    res.json({ type: InteractionResponseType.DeferredChannelMessageWithSource, data: { flags: MessageFlags.Ephemeral } }); handleTranslateText(interaction); return;
  }
  if (name === 'say') {
    res.json({ type: InteractionResponseType.DeferredChannelMessageWithSource, data: { flags: MessageFlags.Ephemeral } }); handleSay(interaction); return;
  }
  if (name === 'voice') {
    res.json({ type: InteractionResponseType.DeferredChannelMessageWithSource, data: { flags: MessageFlags.Ephemeral } }); handleVoice(interaction); return;
  }
  return res.json(ephemeralError('Unknown command.'));
});

app.listen(env.PORT, env.HOST, () => {
  console.log(`TD AI v3.9 listening on ${env.HOST}:${env.PORT}`);
  console.log('Interactions endpoint: /interactions');
  console.log('Private admin dashboard: /admin');

  void startGatewayChat().catch((error) => console.error('Could not start Discord gateway:', error instanceof Error ? error.message : error));
  if (env.REGISTER_COMMANDS_ON_START && env.DISCORD_BOT_TOKEN) {
    void registerGlobalCommands(env.DISCORD_APP_ID, env.DISCORD_BOT_TOKEN).catch((error) => console.error(error instanceof Error ? error.message : error));
  } else {
